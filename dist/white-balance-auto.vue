<template>
  <div class="material-design-icon" id="white-balance-auto-icon" role="img" aria-labelledby="white-balance-auto-title">
    <svg width="24" height="24" viewBox="0 0 24 24">
      <title id="white-balance-auto-title">White balance auto icon</title>
      <path d="M 10.2997,15.9981L 9.5997,13.9981L 6.3997,13.9981L 5.6997,15.9981L 3.7997,15.9981L 6.9997,6.99807L 8.9997,6.99807L 12.1997,15.9981M 21.9997,6.99807L 20.7957,13.2901L 19.2997,6.99807L 17.6997,6.99807L 16.2097,13.2901L 14.9997,6.99807L 14.2387,6.99807C 12.7727,5.17107 10.5247,3.99807 7.9997,3.99807C 3.5817,3.99807 -0.00030009,7.57906 -0.00030009,11.9981C -0.00030009,16.4171 3.5817,19.9981 7.9997,19.9981C 11.1337,19.9981 13.8397,18.1911 15.153,15.5661L 15.2497,15.9981L 16.9997,15.9981L 18.4997,9.89806L 19.9997,15.9981L 21.7497,15.9981L 23.7997,6.99807M 6.8497,12.6481L 9.1497,12.6481L 7.9997,8.99807L 6.8497,12.6481 Z "></path>
    </svg>
  </div>
</template>

<style>
  .material-design-icon {
    display: inline-flex;
    align-self: center;
    position: relative;
    height: 1em;
    width: 1em;

    > svg {
      height: 1em;
      width: 1em;
      bottom: -0.125em;
      position: absolute;
      fill: currentColor;
    }
  }
</style>
